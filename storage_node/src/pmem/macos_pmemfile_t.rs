...
{
    fn from_file<'a>(file_to_map: &str, size: usize, file_open_behavior: FileOpenBehavior) -> Result<Self, PmemError>
    {
	let mut file;
	match file_open_behavior {
            FileOpenBehavior::CreateNew => {
		file = match OpenOptions::new().read(true).write(true).create_new(true).open(file_to_map) {
		    Ok(file) => file,
		    Err(e) => {
            		eprintln!("open: {:?}", e);
